...
describe("fixture-backed Spanish FinanceDashboard", () => {
  ...
  it("keeps the full product boundary outside the UI source", () => {
    ...
    expect(existsSync(resolve(workspace, "apps/api/package.json"))).toBe(true);
    expect(existsSync(resolve(workspace, "apps/web/package.json"))).toBe(true);
    for (const directory of ["backend", "server", "services"]) {
