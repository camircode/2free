...
import type { ProductDataProvider } from "@2free/data-provider/browser";

import { native, type LocalAccount } from "./native";

type AccountType = LocalAccount["type"];
...
export function AccountRegistration({
  ...
  function nextStep(form: HTMLFormElement) {
    ...
    if (fields.every((field) => field.reportValidity())) setStep((current) => current + 1);
  }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setError("");
    try {
      const account = await native.createAccount({
        type,
        label,
        currency,
        metadata: {},
      ...
      if (type === "debit" || type === "yield") {
        await products.create("debit-profile", transferProfile);
      }
      if (type === "yield") {
        await products.create("yield-account", {
          accountId: account.id,
      ...
      if (type === "revolving-credit") {
        ...
        await products.create("credit-card", {
          accountId: account.id,
        ...
        await products.create("notification-rule", {
          name: `Uso mínimo de ${label}`,
          source: `credit-card:${account.id}`,
      ...
      if (type === "charge-card") {
        await products.create("charge-card", {
          accountId: account.id,
          cutoffDay: Number(data.get("cutoffDay")),
          dueDay: Number(data.get("dueDay")),
          annualFee: money(data.get("annualFee"), currency),
          ...
          <div className="account-wizard__step account-wizard__review" hidden={step !== 4}>
            ...
            <dl>
              ...
              <div>
                ...
                <dd>Cuenta y condiciones cifradas en este dispositivo</dd>
              </div>
            </dl>
          </div>
